...
const GuardarAplicacion = async (event) => {
    ...
    const url = '/Guzman_Recuperacion_Dotacion_ingSoft1/aplicaciones/guardar';
    const config = {
        method: 'POST',
        body
    }
...
const BuscarAplicaciones = async () => {
    const url = '/Guzman_Recuperacion_Dotacion_ingSoft1/aplicaciones/buscar';
    const config = {
        method: 'POST'
    }
    ...
    try {
        ...
        if (codigo == 1) {
            ...
            data.forEach((aplicacion, index) => {
                ...
                fila.innerHTML = `
                    ...
                    <td>${aplicacion.app_nombre_mediano}</td>
                    ...
                    <td>
                        <div class='d-flex justify-content-center gap-1'>
                            <button class='btn btn-warning btn-sm modificar' 
                                ...
                                data-medium="${aplicacion.app_nombre_mediano}"  
        ...
        Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se encontraro aplicaciones'
                });
    }
}
...
const llenarFormulario = (event) => {
    ...
    document.getElementById('app_nombre_mediano').value = datos.medium
...
const ModificarAplicacion = async (event) => {
    ...
    const body = new FormData(FormAplicaciones);
    const url = '/Guzman_Recuperacion_Dotacion_ingSoft1/aplicaciones/modificar';
    const config = {
        method: 'POST',
        body
    }
    ...
    try {
        ...
        Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se pudo llenar el formulario'
                });
    }
...
const EliminarAplicacion = async (e) => {
    ...
    if (AlertaConfirmarEliminar.isConfirmed) {
        const url = `/Guzman_Recuperacion_Dotacion_ingSoft1/aplicaciones/eliminar?id=${idAplicacion}`;
        const config = {
            method: 'GET'
        }
