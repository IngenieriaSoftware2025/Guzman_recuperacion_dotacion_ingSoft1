...
const cargarPersonal = async () => {
    const { codigo, data } = await realizarFetch('/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/personal');
    if (codigo == 1) {
        datos.personal = data;
...
const cargarUsuarios = async () => {
    const { codigo, data } = await realizarFetch('/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/usuarios');
    if (codigo == 1) {
        datos.usuarios = data;
...
const verificarControlAnual = async (personalId) => {
    const { codigo, mensaje, data } = await realizarFetch(`/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/controlAnual?personal_id=${personalId}`);
...
const cargarPedidos = async (personalId) => {
    const { codigo, data } = await realizarFetch(`/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/pedidos?personal_id=${personalId}`);
...
const cargarInventario = async (prendaId, tallaId) => {
    const { codigo, data, mensaje } = await realizarFetch(`/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/inventario?prenda_id=${prendaId}&talla_id=${tallaId}`);
...
selPersonal.addEventListener('change', async function() {
    ...
    if (personalId) {
        // COMENTAR TEMPORALMENTE LA VERIFICACIÓN DEL CONTROL ANUAL
        // const puedeRecibir = await verificarControlAnual(personalId);
        // if (puedeRecibir) {
            cargarPedidos(personalId);
        // } else {
...
const guardar = async (e) => {
    ...
    try {
        const { codigo, mensaje } = await realizarFetch('/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/guardar', {
            method: 'POST',
...
const buscar = async () => {
    try {
        const { codigo, mensaje, data } = await realizarFetch('/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/buscar', { method: 'POST' });
...
const modificar = async () => {
    ...
    try {
        const { codigo, mensaje } = await realizarFetch('/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/modificar', {
            method: 'POST',
            body: formData
        });
...
const eliminar = async (e) => {
    ...
    if (confirm.isConfirmed) {
        try {
            const { codigo, mensaje } = await realizarFetch(`/Guzman_Recuperacion_Dotacion_ingSoft1/entregasDot/eliminar?id=${id}`);
