...
permiso_fecha DATETIME YEAR TO DAY,
permiso_situacion SMALLINT DEFAULT 1,
FOREIGN KEY (permiso_app_id) REFERENCES haga_aplicacion(app_id)
);

-- Tabla para Asignación de Permisos
CREATE TABLE haga_asig_permisos(
asignacion_id SERIAL PRIMARY KEY,
asignacion_usuario_id INT NOT NULL,
asignacion_app_id INT NOT NULL,
asignacion_permiso_id INT NOT NULL,
asignacion_fecha DATETIME YEAR TO DAY,
asignacion_quitar_fechaPermiso DATETIME YEAR TO DAY,
asignacion_usuario_asigno INT NOT NULL,
asignacion_motivo VARCHAR(250) NOT NULL,
asignacion_situacion SMALLINT DEFAULT 1,
FOREIGN KEY (asignacion_usuario_id) REFERENCES haga_usuario(usuario_id),
FOREIGN KEY (asignacion_app_id) REFERENCES haga_aplicacion(app_id),
FOREIGN KEY (asignacion_permiso_id) REFERENCES haga_permiso(permiso_id)
);

-- Tabla para Rutas
CREATE TABLE haga_rutas(
...
FOREIGN KEY (ruta_app_id) REFERENCES haga_aplicacion(app_id)
);

-- Tabla para Historial de Actividad
CREATE TABLE haga_historial_act(
...
FOREIGN KEY (historial_usuario_id) REFERENCES haga_usuario(usuario_id),
FOREIGN KEY (historial_ruta) REFERENCES haga_rutas(ruta_id)
);


-- Tabla para Personal para la Dotación 
CREATE TABLE haga_personal_dot(
...
inv_fecha_ing DATETIME YEAR TO DAY,
inv_lote VARCHAR(50),
inv_observ VARCHAR(250),
inv_situacion SMALLINT DEFAULT 1,
FOREIGN KEY (inv_prenda_id) REFERENCES haga_dot_img(prenda_id),
FOREIGN KEY (inv_talla_id) REFERENCES haga_tallas_dot(talla_id)
);

-- Tabla para Pedidos de la Dotación
CREATE TABLE haga_ped_dot(
...
ped_fecha_sol DATETIME YEAR TO DAY,
ped_observ VARCHAR(250),
ped_estado VARCHAR(20) DEFAULT 'PENDIENTE',
ped_situacion SMALLINT DEFAULT 1,
FOREIGN KEY (ped_per_id) REFERENCES haga_personal_dot(per_id),
FOREIGN KEY (ped_prenda_id) REFERENCES haga_dot_img(prenda_id),
FOREIGN KEY (ped_talla_id) REFERENCES haga_tallas_dot(talla_id)
);

-- Tabla para Entregas de  la Dotación
CREATE TABLE haga_ent_dot(
ent_id SERIAL PRIMARY KEY,
ent_per_id INT NOT NULL,
ent_ped_id INT NOT NULL,
ent_inv_id INT NOT NULL,
ent_cant_ent INT NOT NULL,
ent_fecha_ent DATETIME YEAR TO DAY,
ent_usuario_ent INT NOT NULL,
ent_observ VARCHAR(250),
ent_situacion SMALLINT DEFAULT 1,
FOREIGN KEY (ent_per_id) REFERENCES haga_personal_dot(per_id),
FOREIGN KEY (ent_ped_id) REFERENCES haga_ped_dot(ped_id),
FOREIGN KEY (ent_inv_id) REFERENCES haga_inv_dot(inv_id),
FOREIGN KEY (ent_usuario_ent) REFERENCES haga_usuario(usuario_id)
);

-- Tabla para Control Anual de la Dotacion 
CREATE TABLE haga_dot_anual(
...
FOREIGN KEY (dot_anual_per_id) REFERENCES haga_personal_dot(per_id)
);


INSERT INTO haga_usuario (usuario_id,usuario_nom1,usuario_nom2,usuario_ape1,usuario_ape2,usuario_tel,usuario_direc,
...
INSERT INTO haga_aplicacion (app_id,app_nombre_largo,app_nombre_mediano,app_nombre_corto,app_fecha_creacion,app_situacion) 
VALUES(1,'Dotacion','Dotacion','DOTACION','2025-06-18 20:53:31',1)
GO


INSERT INTO haga_permiso (permiso_id,permiso_app_id,permiso_nombre,permiso_clave,permiso_desc,permiso_fecha,permiso_situacion) 
VALUES(1,1,'Admin','ADMIN','Admin','2025-06-18 21:09:47',1)
GO

INSERT INTO haga_permiso (asignacion_id,asignacion_usuario_id,asignacion_app_id,asignacion_permiso_id,asignacion_fecha,asignacion_quitar_fechapermiso,asignacion_usuario_asigno,asignacion_motivo,asignacion_situacion) 
...
usuario que ingresa herberth@gmail.com
contraseña: 12345678

INSERT INTO haga_asig_permisos (
  asignacion_id,
  ...
  2,            -- usuario_id = 2
  1,            -- app_id = 1 (Dotacion)
  1,            -- permiso_id = 1 (ADMIN)
  '2025-07-17', -- fecha actual
  '2026-07-17', -- fecha futura para quitar permiso (puedes cambiarla)
  1,            -- asignado por usuario admin (usuario_id = 1)
  'Asignación total como ADMIN',
  1
);
