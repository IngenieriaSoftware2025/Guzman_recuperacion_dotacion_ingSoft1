    ...
    public static function guardarAPI()
{
    ...
    HistorialActividadesController::registrarActividad(
        '/usuario/guardar',
        'Intento de guardar nuevo usuario del sistema',
        1,
        ['datos_enviados' => array_merge($_POST, ['archivo_subido' => isset($_FILES['usuario_fotografia']) ? $_FILES['usuario_fotografia']['name'] : 'No'])]
    );

    $_POST['usuario_nom1'] = ucwords(strtolower(trim(htmlspecialchars($_POST['usuario_nom1']))));
    if (strlen($_POST['usuario_nom1']) < 2) {
        http_response_code(400);
        echo json_encode(['codigo' => 0, 'mensaje' => 'Primer nombre debe de tener más de 1 caracteres']);
        exit;
    }
    ...
    if (strlen($_POST['usuario_nom2']) < 2) {
        http_response_code(400);
        echo json_encode(['codigo' => 0, 'mensaje' => 'Segundo nombre debe de tener más de 1 caracteres']);
        exit;
    }
    ...
    if (strlen($_POST['usuario_ape1']) < 2) {
        http_response_code(400);
        echo json_encode(['codigo' => 0, 'mensaje' => 'Primer apellido debe de tener más de 1 caracteres']);
        exit;
    }
    ...
    if (strlen($_POST['usuario_ape2']) < 2) {
        http_response_code(400);
        echo json_encode(['codigo' => 0, 'mensaje' => 'Segundo apellido debe de tener más de 1 caracteres']);
        exit;
    }
    ...
    if (strlen((string)$_POST['usuario_dpi']) != 13) {
        http_response_code(400);
    ...
    if (Usuario::fetchFirst("SELECT * FROM haga_usuario WHERE usuario_correo = '{$_POST['usuario_correo']}'")) {
        http_response_code(400);
        echo json_encode(['codigo' => 0, 'mensaje' => 'El correo electrónico ya está registrado']);
        exit;
    }

    if (Usuario::fetchFirst("SELECT * FROM haga_usuario WHERE usuario_dpi = '{$_POST['usuario_dpi']}'")) {
        http_response_code(400);
    ...
    if (isset($_FILES['usuario_fotografia']) && $_FILES['usuario_fotografia']['error'] === 0) {
        ...
        if (!in_array($fileExtension, $allowed)) {
            http_response_code(400);
            echo json_encode(['codigo' => 2, 'mensaje' => 'Solo puede cargar archivos JPG, PNG o JPEG']);
            exit;
        }

        if ($fileSize >= 2000000) {
            http_response_code(400);
            echo json_encode(['codigo' => 2, 'mensaje' => 'La imagen debe pesar menos de 2MB']);
            exit;
        }
    ...
    try {
        ...
        $usuario->usuario_fotografia = $fotoRutaFinal;
        $resultado = $usuario->crear();

        if ($resultado['resultado'] == 1) {
            HistorialActividadesController::registrarActividad(
                '/usuario/guardar',
                'Usuario del sistema guardado exitosamente con ID: ' . $resultado['id'] . ' - DPI: ' . $_POST['usuario_dpi'],
                1,
                ['id_generado' => $resultado['id'], 'dpi' => $_POST['usuario_dpi']]
            );

            http_response_code(200);
            echo json_encode(['codigo' => 1, 'mensaje' => 'Usuario registrado correctamente']);
        } else {
            HistorialActividadesController::registrarError(
                '/usuario/guardar',
                'Error al registrar usuario del sistema',
                1,
                ['resultado' => $resultado, 'dpi' => $_POST['usuario_dpi']]
            );

            http_response_code(500);
            echo json_encode(['codigo' => 0, 'mensaje' => 'Error en registrar al usuario']);
        }
    } catch (Exception $e) {
        HistorialActividadesController::registrarError(
            '/usuario/guardar',
            'Excepción al guardar usuario: ' . $e->getMessage(),
            1,
