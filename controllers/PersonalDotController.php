...
class PersonalDotController extends ActiveRecord
{
    ...
    public static function guardarAPI()
    {
        ...
        $correoExistente = PersonalDot::fetchFirst("SELECT * FROM haga_personal_dot WHERE per_correo = '{$_POST['per_correo']}'");
        if ($correoExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
        ...
        $dpiExistente = PersonalDot::fetchFirst("SELECT * FROM haga_personal_dot WHERE per_dpi = '{$_POST['per_dpi']}'");
        if ($dpiExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
    ...
    public static function modificarAPI()
    {
        ...
        try {
            $sql = "UPDATE haga_personal_dot SET 
    ...
    public static function eliminarAPI()
    {
        ...
        try {
            $sql = "UPDATE haga_personal_dot SET per_situacion = 0 WHERE per_id = $id AND per_situacion = 1";
            $resultado = PersonalDot::getDB()->exec($sql);
