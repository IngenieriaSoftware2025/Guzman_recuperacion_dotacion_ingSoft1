...
class PermisosController extends ActiveRecord
{
    ...
    public static function guardarAPI()
    {
        ...
        $permisoExistente = Permisos::fetchFirst("SELECT * FROM haga_permiso WHERE permiso_clave = '{$_POST['permiso_clave']}' AND permiso_app_id = {$_POST['permiso_app_id']}");
        if ($permisoExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
    ...
    public static function buscarAPI()
    {
        ...
        try {
            ...
            $sql = "SELECT p.permiso_id, p.permiso_app_id, p.permiso_nombre, p.permiso_clave, 
                    ...
                    FROM haga_permiso p 
                    INNER JOIN haga_aplicacion a ON p.permiso_app_id = a.app_id 
    ...
    public static function modificarAPI()
    {
        ...
        $permisoExistente = Permisos::fetchFirst("SELECT * FROM haga_permiso WHERE permiso_clave = '{$_POST['permiso_clave']}' AND permiso_app_id = {$_POST['permiso_app_id']} AND permiso_id != {$_POST['permiso_id']}");
        if ($permisoExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
    ...
    public static function eliminarAPI()
    {
        ...
        try {
            $sql = "UPDATE haga_permiso SET permiso_situacion = 0 WHERE permiso_id = $id AND permiso_situacion = 1";
            $resultado = Permisos::getDB()->exec($sql);
