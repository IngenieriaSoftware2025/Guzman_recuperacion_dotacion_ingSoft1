...
class PrendasDotController extends ActiveRecord
{
    ...
    public static function guardarAPI()
    {
        ...
        $prendaExistente = PrendasDot::fetchFirst("SELECT * FROM haga_dot_img WHERE prenda_nombre = '{$_POST['prenda_nombre']}'");
        if ($prendaExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
    ...
    public static function modificarAPI()
    {
        ...
        $prendaExistente = PrendasDot::fetchFirst("SELECT * FROM haga_dot_img WHERE prenda_nombre = '{$_POST['prenda_nombre']}' AND prenda_id != {$_POST['prenda_id']}");
        if ($prendaExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
    ...
    public static function eliminarAPI()
    {
        ...
        try {
            $sql = "UPDATE haga_dot_img SET prenda_situacion = 0 WHERE prenda_id = $id AND prenda_situacion = 1";
            $resultado = PrendasDot::getDB()->exec($sql);
