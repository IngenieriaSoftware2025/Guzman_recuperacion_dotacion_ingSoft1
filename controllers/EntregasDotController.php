...
class EntregasDotController extends ActiveRecord
{
    ...
    //                 FROM haga_ped_dot p
    //                 LEFT JOIN haga_dot_img pr ON p.ped_prenda_id = pr.prenda_id
    //                 LEFT JOIN haga_tallas_dot t ON p.ped_talla_id = t.talla_id
    ...
    //                 FROM haga_inv_dot i
    //                 LEFT JOIN haga_dot_img pr ON i.inv_prenda_id = pr.prenda_id
    //                 LEFT JOIN haga_tallas_dot t ON i.inv_talla_id = t.talla_id
    ...
    public static function obtenerPedidosAPI()
{
    ...
    public static function guardarAPI()
    {
        ...
        try {
            $sqlStock = "SELECT inv_cant_disp FROM haga_inv_dot WHERE inv_id = {$_POST['ent_inv_id']}";
            ...
            if($resultado['resultado'] == 1){
                $sqlActualizar = "UPDATE haga_inv_dot SET 
                                  inv_cant_disp = inv_cant_disp - {$_POST['ent_cant_ent']}
                                  WHERE inv_id = {$_POST['ent_inv_id']}";
                EntregasDot::getDB()->exec($sqlActualizar);

                $sqlPedido = "SELECT ped_cant_sol FROM haga_ped_dot WHERE ped_id = {$_POST['ent_ped_id']}";
                ...
                if ($totalEntregado['total_entregado'] >= $pedido['ped_cant_sol']) {
                    $sqlActualizarPedido = "UPDATE haga_ped_dot SET ped_estado = 'ENTREGADO' 
    ...
    public static function buscarAPI()
    {
        ...
        try {
            ...
            $sql = "SELECT e.ent_id, e.ent_cant_ent, e.ent_fecha_ent, e.ent_observ,
                           ...
                           p.ped_cant_sol
                    FROM haga_ent_dot e
                    LEFT JOIN haga_personal_dot per ON e.ent_per_id = per.per_id
                    LEFT JOIN haga_ped_dot p ON e.ent_ped_id = p.ped_id
                    LEFT JOIN haga_inv_dot i ON e.ent_inv_id = i.inv_id
                    LEFT JOIN haga_dot_img pr ON i.inv_prenda_id = pr.prenda_id
                    LEFT JOIN haga_tallas_dot t ON i.inv_talla_id = t.talla_id
                    LEFT JOIN haga_usuario u ON e.ent_usuario_ent = u.usuario_id
    ...
    public static function modificarAPI()
    {
        ...
        try {
            $sqlAnterior = "SELECT ent_cant_ent, ent_inv_id FROM haga_ent_dot WHERE ent_id = {$_POST['ent_id']}";
            $entregaAnterior = EntregasDot::fetchFirst($sqlAnterior);
            
            $diferencia = $_POST['ent_cant_ent'] - $entregaAnterior['ent_cant_ent'];
            
            if ($diferencia > 0) {
                $sqlStock = "SELECT inv_cant_disp FROM haga_inv_dot WHERE inv_id = {$entregaAnterior['ent_inv_id']}";
                $inventario = InventarioDot::fetchFirst($sqlStock);
            ...
            if($resultado >= 0){
                $sqlAjustar = "UPDATE haga_inv_dot SET 
    ...
    public static function eliminarAPI()
    {
        ...
        try {
            $sqlEntrega = "SELECT ent_cant_ent, ent_inv_id, ent_ped_id FROM haga_ent_dot WHERE ent_id = $id";
            ...
            $sql = "UPDATE haga_ent_dot SET ent_situacion = 0 WHERE ent_id = $id AND ent_situacion = 1";
            $resultado = EntregasDot::getDB()->exec($sql);
            
            if($resultado > 0){
                $sqlRestaurar = "UPDATE haga_inv_dot SET 
                ...
                $sqlPedido = "SELECT ped_cant_sol FROM haga_ped_dot WHERE ped_id = {$entrega['ent_ped_id']}";
                $pedido = PedidosDot::fetchFirst($sqlPedido);
                
                if ($totalEntregado['total_entregado'] < $pedido['ped_cant_sol']) {
                    $sqlActualizarPedido = "UPDATE haga_ped_dot SET ped_estado = 'APROBADO' 
