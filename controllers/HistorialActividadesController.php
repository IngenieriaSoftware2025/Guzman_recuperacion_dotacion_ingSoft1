...
class HistorialActividadesController extends ActiveRecord
{
    ...
    {
        try {
            $sql = "SELECT ruta_id FROM haga_rutas WHERE ruta_nombre = '{$nombreRuta}' AND ruta_app_id = {$aplicacionId}";
    ...
    public static function buscarAPI()
    {
        ...
        try {
            ...
            $sql = "SELECT 
                    ...
                    FROM haga_historial_act h
                    INNER JOIN haga_rutas r ON h.historial_ruta = r.ruta_id
                    INNER JOIN haga_aplicacion a ON r.ruta_app_id = a.app_id
                    LEFT JOIN haga_usuario u ON h.historial_usuario_id = u.usuario_id
                    WHERE $where 
    ...
    public static function buscarUsuariosAPI()
    {
        ...
        try {
            $sql = "SELECT DISTINCT h.historial_usuario_id, u.usuario_nom1 || ' ' || u.usuario_ape1 as usuario_nombre 
                    FROM haga_historial_act h
                    INNER JOIN haga_usuario u ON h.historial_usuario_id = u.usuario_id
    ...
    public static function buscarAplicacionesAPI()
    {
        ...
        try {
            $sql = "SELECT DISTINCT a.app_id, a.app_nombre_largo as app_nombre 
                    FROM haga_aplicacion a
                    INNER JOIN haga_rutas r ON a.app_id = r.ruta_app_id
                    INNER JOIN haga_historial_act h ON r.ruta_id = h.historial_ruta
